...
export const FillEditor = memo(function FillEditor({ nodeId, fills }: Props) {
  ...
  return (
    <div className="property-grid">
      <div className="fill-add-row">
        ...
        <div className="fill-add-buttons">
          <button
            ...
            onClick={() => addFill(nodeId, 'cross-hatch')}
            title="Volumetric parallel lines"
          >
            Hatch
          </button>
          <button
            className="btn btn-xs"
            onClick={() => addFill(nodeId, 'surface-hatch')}
            title="Face-aligned lines"
          >
            Surface
          </button>
          <button
            className="btn btn-xs"
            onClick={() => addFill(nodeId, 'stipple')}
            title="Poisson dots"
          >
            Dots
          </button>
          <button
            className="btn btn-xs"
            onClick={() => addFill(nodeId, 'contour')}
            title="Level-set planes"
          >
            Contour
          </button>
        </div>
      ...
      {fills.length === 0 && (
        <div
          className="property-row property-row-full"
          style={{ opacity: 0.6, fontStyle: 'italic', fontSize: 10 }}
        >
    ...
    <div className="fill-card">
      ...
      {(fill.type === 'cross-hatch' || fill.type === 'surface-hatch') && (
        <NumberInput
          label="Spacing"
          value={fill.spacing}
          onChange={(v) => onChange({ spacing: Math.max(0.005, v) })}
          step={0.01}
          min={0.005}
        />
      )}
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const next = active
                        ...
                        : [...current, ax];
                      onChange({
                        crossHatchAxes: next.length > 0 ? next : [ax],
                      });
                    }}
                  >
                    {ax.toUpperCase()}
      ...
      {fill.type === 'stipple' && (
        <>
          <NumberInput
            label="Density"
            value={fill.density}
            onChange={(v) => onChange({ density: Math.max(1, v) })}
            step={10}
            min={1}
          />
          <NumberInput
            label="Dot Size"
            value={fill.dotSize}
            onChange={(v) => onChange({ dotSize: Math.max(0.001, v) })}
            step={0.005}
            min={0.001}
          />
          <p className="property-value-text">
            Surface dots sample actual faces; volume dots sample the interior.
          </p>
          <CheckboxInput
            label="Surface"
            value={fill.surfaceMode}
            onChange={(v) => onChange({ surfaceMode: v })}
          />
        </>
      )}
      ...
      <NumberInput
        label="Pen #"
        value={fill.pen}
        onChange={(v) => onChange({ pen: Math.max(1, Math.round(v)) })}
        step={1}
        min={1}
        max={8}
      />
