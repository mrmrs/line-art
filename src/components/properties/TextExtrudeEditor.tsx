...
export const TextExtrudeEditor = memo(function TextExtrudeEditor({
  params: p,
  onChange,
}: Props) {
  ...
  const triggerRegen = (next: Partial<TextExtrudeParams>) => {
    ...
    regenTimer.current = setTimeout(async () => {
      try {
        ...
        if (mounted.current && token === generation.current) onChange({ polylines, bounds });
      } catch (err) {
        if (mounted.current && token === generation.current) reportError(err);
      }
    }, 120);
  };
    ...
    <div className="property-grid">
      ...
      <input
        ...
        onChange={(e) => {
          ...
          if (f) void handleUpload(f).catch(reportError);
          e.target.value = '';
        }}
      />
      ...
      {noFonts ? (
        <div
          ...
          style={{ opacity: 0.85, fontStyle: 'italic', fontSize: 10 }}
        >
          No fonts loaded.
        </div>
      ) : (
      ...
      <NumberInput
        ...
        value={p.fontSize}
        onChange={(v) => {
          const next = Math.max(8, v);
          onChange({ fontSize: next });
          triggerRegen({ fontSize: next });
        }}
        step={10}
        min={8}
      />
      ...
      <NumberInput
        ...
        value={p.lineHeight}
        onChange={(v) => {
          onChange({ lineHeight: Math.max(0.5, v) });
          triggerRegen({ lineHeight: Math.max(0.5, v) });
        }}
        step={0.1}
        min={0.5}
      />
      <SelectInput
        ...
        onChange={(v) => {
          onChange({ align: v });
          triggerRegen({ align: v as 'left' | 'center' | 'right' });
        }}
      />
